...
void USRead()
{
    ...
    digitalWrite(TRIG_PIN, LOW);
    durationSonar = pulseIn(ECHO_PIN, HIGH);
    distanceSonar = (durationSonar * 0.0343) / 2;
    if (distanceSonar % 2 != 0)
    {
        distanceSonar = distanceSonar + 1;
    }
    if (distanceSonar >= 40)
    {
        distanceSonar = 38; // Limits the distance to 40 cm
    }
}
...
void sonar()
{
    if (sonarEnabled)
    {
        ...
        {
            USRead();
            sonarData[i][distanceSonar / 2 -1 ] = 'X';
            sonarData[i][distanceSonar / 2] = 'X';
            sonarData[i][distanceSonar / 2 + 1] = 'X';
            displayArrays();
